use aat_fs::{
    ...
    create_append_file_tool_de, create_create_file_tool_de,
    service::{appendfiletool::AppendFileToolService, createfiletool::CreateFileToolService},
};
...
async fn main() {
    ...
    let createfiletool =
        create_create_file_tool_de(CreateFileToolService(file_tools_adapter.clone()));
    let appendfiletool =
        create_append_file_tool_de(AppendFileToolService(file_tools_adapter.clone()));
...
/* PROMPT:
Hi.
Erstelle eine neue, leere Datei "test.adoc";
Denke dir eine Tabelle in AsciiDoc aus, die die Programmiersprachen Rust, Python und Java gegenüberstellt.
Bette die Tabelle in eine AsciiDoc-Dokument ein und hänge diesen AsciiDoc Quelltext an die soeben erstellte Datei an.
*/
