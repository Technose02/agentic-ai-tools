use crate::{
    ...
    domain::{model::createfiletool::InputParams, port::createfiletool::CreateFileToolInPort},
    error::{Error, TError},
};
...
mod de;
mod en;
pub use de::create_create_file_tool_de;
pub use en::create_create_file_tool_en;

fn create_create_file_tool<P, R, S, T>(description: &'static str, service: S) -> Arc<FunctionTool>
where
    P: TryInto<InputParams, Error = Error> + JsonSchema + Serialize + DeserializeOwned,
    R: From<()> + JsonSchema + Serialize,
    ...
    T: ErrorTranslator + 'static,
{
    ...
    Arc::new(
        FunctionTool::new("create_file_tool", description, handler)
