use crate::{
    ...
    domain::{model::fileexiststool::InputParams, port::fileexiststool::FileExistsToolInPort},
    error::{Error, TError},
};
...
mod de;
mod en;
pub use de::create_file_exists_tool_de;
pub use en::create_file_exists_tool_en;

fn create_file_exists_tool<P, R, S, T>(description: &'static str, service: S) -> Arc<FunctionTool>
where
    P: TryInto<InputParams, Error = Error> + JsonSchema + Serialize + DeserializeOwned,
    R: From<bool> + JsonSchema + Serialize,
    ...
    T: ErrorTranslator + 'static,
{
    ...
    Arc::new(
        FunctionTool::new("file_exists_tool", description, handler)
